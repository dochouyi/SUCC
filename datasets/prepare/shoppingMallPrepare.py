...
class shoppingMallPrepare(BasePrepare):
    def __init__(self, basepath):
        images_path = os.path.join(os.path.abspath(basepath),'images/')
        gts_path = os.path.join(os.path.abspath(basepath),'ground-truth/')
        super(shoppingMallPrepare, self).__init__(img_path=images_path,gts_path=gts_path,ext='*.jpg',is_multi=True, adj_num=3,zoom_para=0.3,down_sample_rate=8)
...
def generate():
    pre=shoppingMallPrepare('/home/houyi/datasets/ShoppingMall/')
    pre.process()
...
def vis():
    ...
    a=Visualize_class(load_gt,max_sigma=100,sigma_rate=0.3)
    a.draw_head_adaptive_theta_circle(image_file='/home/houyi/datasets/ShoppingMall/images/0.jpg', gt_file="/home/houyi/datasets/ShoppingMall/ground-truth/0.txt")
